...
def list_tasks(_tasks):
    #dg574 and 12/06/23 List a summary view of all tasks 
    i = 0
    for t in _tasks:
...
def add_task(name: str, description: str, due: str):
    #dg574 and 12/06/23 here we are adding tasks to the tracker app
    missing_fields = []
    if not name:
...
def process_update(index):
    ...
    #dg574 and 12/06/23 here we can select a task and update any field according to our reqiurements.
...
def update_task(index: int, name: str = None, description: str = None, due: str = None):
    #dg574 and 12/06/23 Here we are updating the existing task 
...
def mark_done(index):
    #dg574 and 12/06/23 this helps to mark a task as done
    if 0 <= index < len(tasks):
        task = tasks[index]
...
def view_task(index):
    #dg574 and 12/06/23 Here we can use this function to view specific task 
    if 0 <= index < len(tasks):
        task = tasks[index]
...
def delete_task(index):
    #dg574 and 12/06/23 here we use this to delete a task 
    if 0 <= index < len(tasks):
        del tasks[index]
...
def get_incomplete_tasks():
    #dg574 and 12/06/23 Prints a list of tasks that are not done.
...
def get_overdue_tasks():
    #dg574 and 12/06/23 Prints a list of tasks that are overdue (not done and expired).
    global tasks
...
def get_time_remaining(index):
    #dg574 and 12/06/23 Outputs the number of days, hours, minutes, seconds a task has before it's overdue or how far past due it is 
    if 0 <= index < len(tasks):
        task = tasks[index]
        now = datetime.now()
